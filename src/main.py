...
modified by Ji Yu Moon, Hee Jeong Lim, Do Hyeon Jeong
"""
...
def main(config):
    ...
    my_data = data_class(root_dir = config['data_dir'], pattern=config['pattern'], n_proc=1, config=config)
    # 출력 코드
    ...
    # 확인용 코드
    logger.info('train_indices : ', train_indices)
    logger.info('val_indices : ', val_indices)
    logger.info('test_indices', test_indices)
    ...
    if args.load_model:
        model, optimizer, start_epoch = utils.load_model(model, config['load_model'], optimizer, config['resume'],
                                                         ...
                                                         config['lr_step'],
                                                         config['lr_factor'])
        print('Sleep Parmeters : ', model.sleep_enc.sleep)
    model.to(device)

    loss_module = get_loss_module(config)

    # Only for Test, skip training
    if config['test_only'] == 'testset': 
        ...
        aggr_metrics_test, per_batch_test = test_evaluator.evaluate(epoch_num=0, keep_all=True)
    ...
    # Initialize data generators + masking 수행
    dataset_class, collate_fn, runner_class = pipeline_factory(config)
